...
const CONTRACT_ABI = [
  "function submitPatientData(bytes32 encryptedGlucose, bytes calldata inputProof, string calldata ipfsCid, string calldata loincCode) external",
...
export const ContractProvider = ({ children }) => {
  const { signer, account, isSepoliaNetwork } = useWallet()
  ...
  // 患者提交数据
  const submitPatientData = async (glucoseValue, ipfsCid, loincCode = "2345-7") => {
    if (!contract || !account) {
    ...
    setLoading(true)
    try {
      // 注意：这里需要实际的 FHE 加密，目前使用模拟数据
      const mockEncryptedGlucose = ethers.keccak256(ethers.toUtf8Bytes(glucoseValue.toString()))
      const mockProof = "0x"

      const tx = await contract.submitPatientData(
        mockEncryptedGlucose,
        mockProof,
        ipfsCid,
        loincCode
      )
